...
export default function RsvpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (formData.partyCount > MAX_PARTY_SIZE || formData.partyCount < 1) {
      setPartyError(`Party size cannot exceed ${MAX_PARTY_SIZE} people (including yourself).`);
      return;
    }
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (val > MAX_PARTY_SIZE) {
                  setPartyError(`Maximum ${MAX_PARTY_SIZE} people permitted per party.`);
                } else if (val < 1) {
